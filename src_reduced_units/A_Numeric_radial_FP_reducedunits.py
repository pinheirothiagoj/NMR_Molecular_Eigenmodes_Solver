...
# magnetic dipoles in reduced units. This is achieved by numerically solving
...
potential_def = "analytical"  # "read" or "analytical"
...
T,D0,ri,rf,dr,Km,dx,Kp,req = read_input_file(filename)
...
if potential_def == "analytical":
    ...
    r_vals = data[:, 0]    # dimensionless (r/sig)
...
for i in range(0,Nx):
    ...
    if abs(hi[i]) + abs(hi[i-1]) != abs(hi[i]+hi[i-1]) and abs(hf[i]) + abs(hf[i-1]) != abs(hf[i]+hf[i-1]):
        ...
        if count>=1:
            ...
            print("Eigenvalue = "+"{:.10E}".format(root[-1]),"|","Tau = "+"{:.10E}".format(tau[-1]))
...
for i in range(0,lidx):
    ...
    for j in range(0,Nr): # r0 integration
        ...
        Ptau[i] += prefac * np.exp(-U[j]/(kB*T)) * Arg / r[j] * dr
...
header = "r \t" + "\t".join([f"R_{d+1}(r) \t" for d in range(lidx)])
print(header)
for i in range(0,Nr):
